...
'{text}'
Rules:
1. Provide ONLY the direct English translation.
...
5. Your response must contain ONLY the English translation, nothing else."""
    ...
    translation = re.sub(r'^(Here\'s the translation:|Here is the translation:|Here\'s the direct English translation:)', '', translation, flags=re.IGNORECASE)
...
def process_vtt_file(file_path):
    ...
    for block in blocks:
        ...
        if len(lines) >= 2 and re.match(r'\d{2}:\d{2}:\d{2}\.\d{3} --> \d{2}:\d{2}:\d{2}\.\d{3}', lines[0]):
            ...
            translated_text = translate_portuguese_to_english(text_to_translate)
